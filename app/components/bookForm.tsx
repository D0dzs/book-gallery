...
export default function BookForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const res = await fetch(
      `api/v1/recordbook?bWriter=${encodeURIComponent(writer)}&bTitle=${encodeURIComponent(title)}&bGenre=${encodeURIComponent(genre)}&bReleaseDate=${encodeURIComponent(releaseDate)}`,
      {
        method: "POST",
      }
    );
